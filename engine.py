...
class Entity(pygame.sprite.Sprite):
    ...
    def draw(self, display, scroll=[0,0], color=(79, 79, 79)):
        ...
        if self.image == '':
            pygame.draw.rect(display, color, (self.rect.x-scroll[0], self.rect.y-scroll[1], self.rect.w, self.rect.h))
        else:
            self.rect2.center = self.rect.center
            display.blit(self.image, (self.rect2.x-scroll[0], self.rect2.y-scroll[1]))
            
        if self.debug:
            pygame.draw.rect(display, (0, 250, 83), (self.rect.x-scroll[0], self.rect.y-scroll[1], self.rect.w, self.rect.h), 1)
...
class SimplePlayer(RigidyBory):
    def __init__(self, pos, size, image, movement_speed, gravity_acceleration, jump_force, count_jump, debug):
...
class TileMap():
    def __init__(self, image, tiles, pos, colliders_list):
...
def scroll_player(player, display_size, scroll, delay, first_time):
    if first_time:
        delay = 1
        first_time = False
